    ...
    for i in range(N):
        for j in range(C):
            for k in range(H_out):
                for l in range(W_out):
                    image_patch =  x[i, j, :, :]
                    out[i, j, k, l] = np.max(image_patch[(k*S):(k*S + h), (l*S):(l*S + w)])
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(C):
            ...
            for k in range(H_out):
                for l in range(W_out):
                    image_patch = image[(k*S):(k*S+h),(l*S):(l*S+w)]
                    max_index = np.where(image_patch == image_patch.max())
                    max_index = tuple(map(sum, zip((k*S, l*S), max_index))) 
