...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    """subt = self.X_train[:,np.newaxis] - X
    print(np.shape(subt))
    dists = np.linalg.norm(subt,axis = 2)
    dists = self.compute_distances_one_loop(X)
    """
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      #########################################################################
      arranged = np.argsort(dists[i])
      for l in range(k):
        closest_y.append(self.y_train[arranged[l]])
      #########################################################################
      # TODO:                                                                 #
